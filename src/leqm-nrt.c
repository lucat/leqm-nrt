...
main (int argc, const char **argv)
{
  ...
  for (int in = 1; in < argc;)
  {
	...
	if (lkfs)
	{
	  ...
	  channelgateconfvector = malloc (sizeof (int) * sfinfo.channels);
	}
...
//postprocessing parameters
  ...
  if (lkfs)
  {
    ...
    LGCtx->totalsteps =
      ((int)
       (18000.00 /
	(((double) LGCtx->ops) / ((double) codecContext->sample_rate)))) + 1;
...
if (!poly) {
  ...
  {
    printf ("%d\t%.2f\t%.2f\t%.2f\n", i, eqfreqsamples[i], eqfreqresp_db[i],
	    eqfreqresp[i]);
  }
#endif
...
// write a function that uses get to change data to double and copy in worker buffer
		...
		for (int ch_index = 0; ch_index < nchannels; ch_index++)
		{
		  ...
		  if (worker_id == min (nchannels, numCPU))
		  {
		    ...
		    for (int idxcpu = 0;
			 idxcpu < min (nchannels, numCPU); idxcpu++)
		    {
	...
	WorkerArgsArray[worker_id]->fullbuffer_nsamples = buffersizesamplesdi;
	WorkerArgsArray[worker_id]->nch = codecContext->channels;
	WorkerArgsArray[worker_id]->npoints = npoints;
	WorkerArgsArray[worker_id]->ir = ir;
	WorkerArgsArray[worker_id]->ptrtotsum = totsum;
  ...
  print_di (codecContext->channels, realnumbershortperiods,
	    shorttermdidecisionarray);
#elif defined SNDFILELIB
  print_di (sfinfo.channels, numbershortperiods, shorttermdidecisionarray);
#endif
...
// write a function that uses get to change data to double and copy in worker buffer
	  ...
	  while (copiedsamples >= buffersizesamples)
	  {
...
#endif
	      pthread_create (&tid[worker_id], &attr,
			      worker_function, WorkerArgsArray[worker_id]);
#ifdef DI
	    }
#endif
	    worker_id++;
		...
		meanoverduration (totsum);	//update leq(m) until now and log it
		logleqm (leqmlogfile,
			 ((double) totsum->nsamples) /
			 ((double) codecContext->sample_rate), totsum);
	      }			//endlog
	    }			//if (worker_id == numCPU)
...
if (leqm10)
{
  ...
  if (dolbydi)
  {
    WorkerArgsArray[worker_id]->sc_shorttermarray = sc_shorttermaveragedarray;
  }
#endif
}
else
...
if (worker_id == numCPU)
{
  ...
  if (leqmlog)
  {
    meanoverduration (totsum);	//update leq(m) until now and log it
    logleqm (leqmlogfile,
	     ((double) totsum->nsamples) / ((double) sfinfo.samplerate),
	     totsum);
  }				//endlog
}
    //end while worker_id
...
if ((dolbydi) && !(lkfs))
{
  ...
  if (levelgated)
  {
    ...
    levelgatefinalcomputation (sc_shorttermaveragedarray,
			       leqmtosum (levelgatedthreshold, 0.000020),
			       sfinfo.channels, numbershortperiods, totsum);
#endif
    printf ("Leq(M,LG): %.4f\n", totsum->lgleqm);
  }
  printf ("Leq(M,DI): %.4f\n", totsum->dgleqm);
...
if (leqm10)
{
  ...
  double *allenmetricarray =
    malloc (sizeof (*allenmetricarray) * (realnumbershortperiods - rollint));
#elif defined SNDFILELIB
  double *allenmetricarray =
...
void *
worker_function_gated (void *argstruct)
{
...
void
logleqm (FILE * filehandle, double featuretimesec, struct Sum *oldsum)
{
  ...
  if (thisWorkerArgs->samples_read_array[thisWorkerArgs->channel] > 0)
  {
    output =
      di_process (thisWorkerArgs->di_array[thisWorkerArgs->channel],
		  ...
		  thisWorkerArgs->
		  samples_read_array[thisWorkerArgs->channel]);
    if (output == INVALID)
    {
      thisWorkerArgs->buffered_samples_array[thisWorkerArgs->channel] +=
	thisWorkerArgs->samples_read_array[thisWorkerArgs->channel];
				   ...
				   uint8_t ** stdda, double adlgthreshold)
{
  ...
  if (dialoguepercentage >= adlgthreshold)
  {
    ...
    DILKFS = -0.691 + 10 * log10 (DI_LKFS_accum / ((double) digatedcounter));
    printf ("LKFS(DI): %.2f\n", DILKFS);
  }
  free (ch_accumulator);
